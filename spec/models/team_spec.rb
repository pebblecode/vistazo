...
describe "Team model" do
  ...
  describe "user_timetables_in_month" do
    before do
      @team.add_user(@user)
      ...
      @next_month_timetable_item = @team.add_timetable_item(@user, @project, @next_month_date)

      @team.reload
    end

    it "should return the timetable item in the month" do
      user_timetables = @team.user_timetables_in_month(@date_month)
      user_timetables.length.should == 1
      timetable_items = user_timetables.first.timetable_items
      timetable_items.length.should == 1
    ...
    it "should not return the timetable item in the next month" do
      user_timetables = @team.user_timetables_in_month(@next_month)
      user_timetables.length.should == 1
      timetable_items = user_timetables.first.timetable_items
      timetable_items.length.should == 1

      timetable_items.include?(@timetable_item).should == false
      timetable_items.include?(@next_month_timetable_item).should == true
    end
  end
